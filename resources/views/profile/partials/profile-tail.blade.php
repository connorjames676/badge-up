<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">Attempts and Comments</h2>
    </header>

    <div class="mt-3">
        @forelse ($items as $item)
            @if ($item instanceof App\Models\Attempt)
                <div class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight" style="margin-bottom: 3px;">
                    <article class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <h4 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                                    <a href="{{ route('attempts.show', $item->id) }}">{{ $item->title }}</a>
                                </h4>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    by <a href="{{ route('profile.show', $user->id) }}">{{ $item->user->name ?? 'Unknown' }}</a>
                                </p>
                            </div>
                        ...
                        <div class="mt-4 flex w-full items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                            @php $liked = $item->likes->contains('user_id', auth()->user()->id); @endphp
                            @if ($liked)
                            ...
                            <a href="{{ route('comments.create', $item->id) }}" class="hover:text-indigo-600">Comment</a>
                        ...
                        <div class="font-semibold mt-4 flex w-full items-center gap-3 text-m text-gray-500 dark:text-gray-400">
                            @if (auth()->user()->id == $item->user_id)
                                <div class="mt-4 flex w-full items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                                    <a href="{{ route('attempts.edit', $item->id) }}">Edit</a>
                                    <form method="POST" action="{{ route('attempts.destroy', $item->id) }}">
                                        @csrf
                                        @method('DELETE')
                ...
                <div class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    <article class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800" style="margin-bottom: 3px;">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <p class="mt-3 text-sm text-gray-700 dark:text-gray-200 line-clamp-3">
                                    <a href="{{ route('attempts.show', $item->attempt_id) }}">
                                        {{ $item->content }}
                                    </a>
                                </p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    by <a href="">{{ $item->user->name ?? 'Unknown'}}</a>
                                </p>
                            </div>
                            <div>
                            <a href="{{ route('comments.edit', [$item->attempt, $item]) }}">
                                <p class="mt-3 text-sm text-gray-700 dark:text-gray-200 line-clamp-3">Edit</p>
                            </a>

                            <form method="POST" action="{{ route('comments.destroy', [$item->attempt, $item]) }}">
                                @csrf
                                @method('DELETE')
